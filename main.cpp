...
autocore get_obj() {
  boost::filesystem::path p = boost::process::search_path("duplicity");
...
int main() {
  ...
  if (!config) {
    ...
    autocore dup(
        ...
        root.get<std::string>("signPassphrase"), root.get<std::string>("path"));
    bg::date dd =
    ...
    if (isFull) {
      ...
      uint64_t temp = root.get<uint64_t>("totalIncrSize");
      root.put("totalIncrSize", sz + temp);
    }
    std::ofstream file;
